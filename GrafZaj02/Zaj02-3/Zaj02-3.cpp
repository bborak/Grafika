...
static GLboolean iCull, iDepth, iOutline, iSmooth;
...
void SetupRC()
{
	...
	// Czarne t³o
	glClearColor(1.0f, 0.1f, 1.5f, 1.0f);
	// £adowanie tekstury
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, 256, 256, 50, GL_LUMINANCE, GL_UNSIGNED_BYTE, pixels);
...
void RenderScene(void)
{
	...
	for (angle = 0.0f; angle < (2.0f*GL_PI); angle += (GL_PI / 7.0f))
	{
	...
	for (angle = 0.0f; angle < (3.0f*GL_PI); angle += (GL_PI / 7.0f))
	{
		// Wyliczenie wspó³rzêdnych x i y kolejnego wierzcho³ka
		x = 50.0f*sin(angle);
		y = 50.0f*cos(angle);
...
void Menu(int value)
{
	...
	case GL_SMOOTH:
		iSmooth = !iSmooth;
		glutSetWindowTitle("Stozek - GL_smoothtest");
		RenderScene();
		break;
...
int main(int argc, char* argv[])
{
	...
	//glutAddMenuEntry("smoth", GL_SMOOTH);
