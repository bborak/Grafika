// Zaj1_5.cpp : Defines the entry point for the console application.
//
...
void RenderScene(void)
{
	...
	for (int n = 0; n < punkty.size(); n++)
		glVertex2f(punkty[n].x, punkty[n].y);
	glEnd();
...
void onMouseDown(int button, int state, int x, int y)
{
	...
	{
		punkty.push_back(myVECTOR(x, glutGet(GLUT_WINDOW_HEIGHT) - y));
	}
	glutPostRedisplay();
