...
void GenerateTextures()
{
	...
	// wczytanie tekstury blackman.tga
	GLboolean error = load_targa("face.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku face.tga");
		exit(0);
	}
	...
	// wczytanie tekstury blackman.tga
	error = load_targa("face_gray.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku face_gray.tga");
		exit(0);
	}
...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("face.tga (kompresja)", TEXTURE_LENA);
	glutAddMenuEntry("face.tga (bez kompresji)", TEXTURE_LENA_UNC);
	glutAddMenuEntry("face_gray.tga (kompresja)", TEXTURE_LENA_GRAY);
	glutAddMenuEntry("face_gray.tga (bez kompresji)", TEXTURE_LENA_GRAY_UNC);
