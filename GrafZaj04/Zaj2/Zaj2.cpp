// Zaj2.cpp : Defines the entry point for the console application.
//
...
void GenerateTextures()
{
	...
	GLboolean error = load_targa("GRASS_2.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku GRASS_2.tga");
		exit(0);
	}
	...
	error = load_targa("TREE_CHERRY.tga", width, height, format, type, pixels);

	// błąd odczytu pliku
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku TREE_CHERRY.tga");
		exit(0);
	}
